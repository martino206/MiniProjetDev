...
exports.updateProfile = async (req, res, next) => {
  try {
    ...
    const [user] = await db.query('SELECT u.*, r.name as role_name FROM users u JOIN roles r ON u.role_id = r.id WHERE u.id = ?', [req.user.id]);
...
exports.updateAvatar = async (req, res, next) => {
  try {
    ...
    if (current[0]?.avatar) {
      const oldPath = path.join('.', current[0].avatar);
    ...
    await db.query(
      ...
      [req.user.id, req.file.filename, req.file.originalname, req.file.mimetype, req.file.size, 'avatar']
    );
...
exports.getAllUsers = async (req, res, next) => {
  try {
    ...
    const [[{ total }]] = await db.query(
      'SELECT COUNT(*) as total FROM users u WHERE u.username LIKE ? OR u.email LIKE ?',
      [`%${search}%`, `%${search}%`]
    );
